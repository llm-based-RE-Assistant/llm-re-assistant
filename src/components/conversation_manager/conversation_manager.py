...
from src.components.requirement_extractor import RequirementExtractor, create_extractor
...
from src.components.conversation_manager.utils import _message_similarity, _SMART_CHECK_PROMPT
...
class ConversationManager:
    ...
    def _run_smart_check(self, extracted: list, user_message: str) -> list:
        ...
        prompt = _SMART_CHECK_PROMPT.format(
    ...
    def send_turn(self, user_message: str, state: ConversationState,
        ...
        if self._domain_discovery and state.domain_gate:
            ...
            if next_turn_id == 1 and not has_existing_domains:
                # Fresh seed: use customer's first message
                self._domain_discovery.seed(
                    user_message, state.domain_gate, next_turn_id,
                    project_name=state.project_name)
            elif next_turn_id == 1 and has_existing_domains:
                # Pre-loaded domains (uploaded reqs): reseed with first message as string
                self._domain_discovery.reseed(
                    user_message, state.domain_gate, state, next_turn_id)
            elif next_turn_id == DomainDiscovery.RESEED_TURN:
                all_user_msgs = " ".join(t.user_message for t in state.turns) + "\n" + user_message
                self._domain_discovery.reseed(
                    all_user_msgs, state.domain_gate, state, next_turn_id)
                ...
                # Only for medium/complex — skip simple systems
            ...
            # P2. Project name resolution (needs LLM; only on first turn)
            if state.project_name_needs_llm and next_turn_id == 1:
            ...
            # Runs pre-call so the complexity label is in the system prompt from the
            # turn immediately after seeding — not delayed by one turn.
            if (not getattr(state, "system_complexity", "")
                    and state.domain_gate.seeded
                    and state.domain_gate.total >= 3):
                _desc = " ".join(t.user_message for t in state.turns[-5:]) + " " + user_message
            ...
            # Generated once per domain, as soon as the domain has >=1 probe and the
            # template does not yet exist. Running pre-call means the checklist is
            # injected into THIS turn's system prompt, not the next one.
            # probe_count is updated post-call (step 5j), so we check >= 0 here
            # (any probe) to avoid a one-turn lag; the first call generates the
            # template from whatever requirements exist so far.
