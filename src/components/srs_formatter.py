...
class SRSFormatter:
    ...
    # ------------------------------------------------------------------
    # Header  (Iteration 4: dual metrics — Priority 5)
    # ------------------------------------------------------------------
    ...
    def _render_header(
        ...
        w("| Status | Generated by RE Assistant Iteration 4 |")
    ...
    def _render_appendix_b(
        ...
        _STATUS_LABEL = {
            ...
            DOMAIN_STATUS_EXCLUDED:  "❌ Excluded (out of scope)",
    ...
    # ------------------------------------------------------------------
    # Appendix D — Design-Derived Requirements Inventory (Priority 4)
    # ------------------------------------------------------------------
    ...
    def _render_appendix_d(
        ...
        """
        Priority 4: Inject [D — Architecture Review Required] stubs for every
        functional domain and IEEE-830 section that has no elicited content.

        These stubs do NOT invent fake requirements. They mark the gap
        explicitly so architects and domain experts know what to fill in.
        Tagged [D] to match the ground-truth convention for design-derived
        requirements.
        """
        lines += _section_divider(
            "Appendix D — Design-Derived Requirements Inventory"
        )
        lines.append(
            ...
            "the stakeholder interview alone. Each entry identifies a gap area "
            "and the type of requirement expected there.*"
        )
        lines.append("")
        ...
        _DOMAIN_EXPECTED: dict[str, list[str]] = {
            "climate_control": [
                "Thermostat hardware specification (sensor range, precision, count)",
                "Temperature set-point constraints (min/max values, increment)",
                "Humidistat hardware specification (sensor range, precision, count)",
                "Humidity set-point constraints (min/max %, increment)",
                "HVAC compatibility standards (e.g., ASHRAE)",
                "Manual override persistence rule for thermostat/humidistat",
            ],
            "security_alarm": [
                "Maximum number of contact sensors the system can manage",
                "Alarm activation rule (trigger condition: contact sensor OPEN)",
                "Alarm subsystem specification (sound alarm + light alarm)",
                "Security breach logging requirement",
            ],
            "appliance_lighting": [
                "Maximum number of power switches the system can manage",
                "Power switch specification (voltage/ampere rating)",
                "Power switch state persistence rule (manual vs. scheduled)",
                "Appliance state polling / data acquisition rate",
            ],
            "scheduling_planning": [
                "Monthly planner structure (days × time periods × parameters)",
                "Maximum time periods per day per device",
                "Override mechanism for planned settings",
                "Historical plan retention period",
            ],
            "remote_access": [
                "Supported client platforms (web browser, iOS app, Android app)",
                "Communication protocol between client and home server",
                "Home web server specification (role, capabilities)",
                "Master control gateway device specification",
                "Broadband ISP and connection type requirements",
                "Wireless transmission range requirement for gateway",
            ],
            "user_management": [
                "User role hierarchy (general user / master user / technician)",
                "Account creation procedure at system installation",
                "Login credential requirements (account name + password)",
                "Master user privileges (system configuration, adding accounts)",
                "Technician role privileges (setup, start/stop, parameter setting)",
            ],
            "reporting_history": [
                "Monthly report structure (daily avg/max/min per sensor)",
                "Report retention period (e.g., two years of data)",
                "Security breach log entries in monthly report",
                "System downtime/non-operation log in monthly report",
                "Sensor data acquisition and storage rate",
            ],
            "hardware_connectivity": [
                "RF module specification for gateway ↔ device communication",
                "Wireless transmission range (indoor, e.g., 1000 feet)",
                "Sensor polling/data acquisition rate (e.g., 10 Hz minimum)",
                "Display refresh rate for environmental conditions (e.g., ≤2s)",
                "Device proximity constraint (max distance from gateway)",
            ],
        }

        _DOMAIN_IEEE_MAP: dict[str, str] = {
            "climate_control":      "§3.1 / §3.4 Sensors & Controllers",
            "security_alarm":       "§3.1 / §3.4 Sensors & Controllers",
            "appliance_lighting":   "§3.1 / §3.4 Sensors & Controllers",
            "scheduling_planning":  "§3.1 Functional / §3.2 Planning",
            "remote_access":        "§3.2 External Interfaces",
            "user_management":      "§3.1 Functional / §5.4 Security",
            "reporting_history":    "§3.1 Functional / §3.6 Reporting",
        ...
        for domain_key, spec in DOMAIN_COVERAGE_GATE.items():
            ...
            expected = _DOMAIN_EXPECTED.get(domain_key, [])
            if not expected:
                continue

            status_note = {
                DOMAIN_STATUS_EXCLUDED: "❌ Excluded by stakeholder — confirm before ruling out",
                DOMAIN_STATUS_PARTIAL:  "🔶 Partially elicited — incomplete",
                DOMAIN_STATUS_UNPROBED: "⬜ Never elicited — architect review required",
            }.get(status, "")

            ieee_ref = _DOMAIN_IEEE_MAP.get(domain_key, "§3.x TBD")

            lines.append(f"#### {spec['label']}")
            lines.append(f"*Status: {status_note} | IEEE-830 ref: {ieee_ref}*")
            lines.append("")

            for i, expected_req in enumerate(expected, 1):
                stub_id = f"D-{domain_key[:4].upper()}-{i:02d}"
                lines.append(
                    f"**[D] {stub_id}** — `[Architecture Review Required]`"
                )
                lines.append(
                    f"*Expected requirement area:* {expected_req}"
                )
                lines.append(
                    "*Action:* Confirm with system architect whether a formal "
                    "requirement exists here. If yes, elicit the specific "
                    "constraint and add it to §3 before development begins."
                )
                lines.append("")
                stub_count += 1
        ...
        # ── IEEE-830 structural stubs for completely unelicited sections ─
        ...
        _SECTION_EXPECTED: dict[str, tuple[str, str]] = {
            "maintainability": (
                ...
                "commercial version compatibility rules.",
            ),
            "compatibility": (
                "§3.6.5 Portability / Compatibility",
                "Supported hardware standards, software platform constraints, "
                "technology-choice restrictions.",
            ),
            "constraints": (
                "§3.5 Design Constraints",
                "Development methodology (e.g., OOP/UML), required IEEE standards, "
                "document format standards, technology constraints.",
            ),
        }
        ...
        for cat_key, (section_label, expected_content) in _SECTION_EXPECTED.items():
            ...
            lines.append(
                "*Action:* Review with system architect and add formal "
                "requirements to the appropriate section before development.*"
            )
            lines.append("")
        ...
        lines.append("")
        lines.append("| Category | Stub Count | Action Priority |")
        lines.append("|----------|-----------|-----------------|")
        lines.append(
            f"| Unprobed functional domains | "
            f"{sum(1 for s in gate_status.values() if s == DOMAIN_STATUS_UNPROBED)} domains | 🔴 Critical |"
        )
        lines.append(
            f"| Partially elicited domains | "
            f"{sum(1 for s in gate_status.values() if s == DOMAIN_STATUS_PARTIAL)} domains | 🟡 High |"
        )
        lines.append(
            f"| Excluded domains (confirm) | "
            f"{sum(1 for s in gate_status.values() if s == DOMAIN_STATUS_EXCLUDED)} domains | 🟢 Low |"
        )
        lines.append(
            f"| Structural IEEE-830 stubs | {structural_stub_count} sections | 🟡 High |"
        )
        lines.append(
        ...
        lines.append(
            ...
            "Running a follow-up elicitation session targeting the unprobed "
            "domains is strongly recommended before finalising this SRS."
        )
        lines.append("")
