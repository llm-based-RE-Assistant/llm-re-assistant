...
Uses StubProvider for any LLM calls that flow through the build pipeline.
...
# ---------------------------------------------------------------------------
# build_system_message — FR phase (real state, real prompt build)
# ---------------------------------------------------------------------------
...
class TestBuildSystemMessageNFRPhase:
    def test_nfr_phase_label_present(self):
        a = PromptArchitect()
        s = _state(
            functional=MIN_FUNCTIONAL_REQS,
            domain_gate=_satisfied_gate(),
            nfr_coverage={},
        )
        result = a.build_system_message(s)
    ...
    def test_nfr_context_in_message(self):
        ...
        result = a.build_system_message(s)
        # NFR context includes category names
...
class TestPublicHelpers:
    def test_get_category_labels_returns_dict(self):
        a = PromptArchitect()
        result = a.get_category_labels()
        assert isinstance(result, dict)

    def test_get_category_labels_matches_ieee830(self):
        a = PromptArchitect()
        assert a.get_category_labels() == dict(IEEE830_CATEGORIES)
    ...
    def test_get_mandatory_nfr_categories(self):
        a = PromptArchitect()
        result = a.get_mandatory_nfr_categories()
        assert isinstance(result, frozenset)
        assert result == MANDATORY_NFR_CATEGORIES

    def test_get_min_functional_reqs(self):
        a = PromptArchitect()
        assert a.get_min_functional_reqs() == MIN_FUNCTIONAL_REQS
...
class TestDetermineElicitationPhase:
    def test_fr_when_gate_none_and_no_nfr(self):
        ...
        assert determine_elicitation_phase(s) == "nfr"  # gate=None → domain_ok=True
